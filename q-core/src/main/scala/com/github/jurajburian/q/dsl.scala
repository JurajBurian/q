...
/** Bind iterable value.
  ...
  * @param value
  */
private[q] class BASV(value: Iterable[?]) extends QTransformable {
...
/** Bind Array value
  */
extension (value: (Array[?] | Iterable[?])) {
  @targetName("question")
  def ? : QTransformable = value match {
    case arr: Array[?]   => new BASV(immutable.ArraySeq.unsafeWrapArray(arr))
    case it: Iterable[?] => new BASV(it)
  }
}
